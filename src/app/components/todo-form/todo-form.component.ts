...
export class TodoFormComponent {
  ...
  private dialogService = inject(
    MatDialogRef<HeaderComponent>,
  );
  ...
  public hanleCreateNewTodo(): void {
    if (this.todosForm.value && this.todosForm.valid) {
      const title = String(
        this.todosForm.controls['title'].value,
      );
      const description = String(
        this.todosForm.controls['description'].value,
      );

      const id =
        this.allTodos.length > 0
          ? this.allTodos.length + 1
          : 1;
      const done = false;
